...
class NEAudioService extends _Service {
  ...
  Future<Set<NEAudioOutputDevice>> enumAudioDevices() async {
    ...
    return (devices as List)
        .map((e) => NEAudioOutputDeviceExtension.fromValue((e as num).toInt()))
        .toSet();
  }
  ...
  Future<NEAudioOutputDevice> getSelectedAudioDevice() async {
    ...
    return NEAudioOutputDeviceExtension.fromValue((device as num).toInt());
  }
  ...
  void _ensureEnableAudioDeviceSourceStream() {
    if (_sourceStream == null) {
      _sourceStream =
        ...
        assert(event is Map);
        _curEvent = (
          NEAudioOutputDeviceExtension.fromValue(
              (event['selectedAudioDevice'] as num).toInt()),
          (event['availableAudioDevices'] as List)
              .map((e) =>
                  NEAudioOutputDeviceExtension.fromValue((e as num).toInt()))
              .toSet(),
