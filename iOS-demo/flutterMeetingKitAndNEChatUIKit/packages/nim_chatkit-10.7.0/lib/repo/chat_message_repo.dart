...
class ChatMessageRepo {
  ...
  ///回复消息
  static Future<NIMResult<NIMSendMessageResult>> replyMessage(
  ...
  static Future<NIMResult<NIMSendMessageResult>> sendTextMessageWithMessageAck(
      ...
      NIMMessageAIConfigParams? aiConfigParams}) async {
  ...
  ///收藏消息
  @Deprecated('user [addCollectMessage] instead')
  ...
  ///根据Refer 获取一条消息
  static Future<NIMResult<NIMMessage>> getMessageByRefer(
      NIMMessageRefer messageRefer) {
    return NimCore.instance.messageService
        .getMessageListByRefers(messageRefers: [messageRefer]).then((result) {
      if (result.isSuccess && result.data != null) {
        return NIMResult.success(data: result.data!.first);
      } else {
        return NIMResult.failure(
            message: result.errorDetails, code: result.code);
      }
    });
  }

  static Future<NIMResult<List<ChatMessage>>> getMessageListEx(
      NIMMessageListOption option,
      {bool enablePin = true,
      bool addUserInfo = true}) async {
    var msgRes = await NimCore.instance.messageService.getMessageListEx(option);
    if (msgRes.isSuccess &&
        msgRes.data != null &&
  ...
  ///搜索消息
  static Future<NIMResult<NIMMessageSearchResult>> searchMessageEx(
      NIMMessageSearchExParams params,
      {int limit = 100}) async {
    if (params.limit == null) {
      params.limit = limit;
    }
    if (await IMKitClient.enableCloudMessageSearch) {
      return NimCore.instance.messageService.searchCloudMessagesEx(params);
    } else {
      return NimCore.instance.messageService.searchLocalMessages(params);
    }
  }

  ///根据消息类型获取消息列表
  static Future<NIMResult<NIMMessageListResult>> getMessageListByType(
      List<NIMMessageType> messageType, String conversationId,
      {int? limit,
      NIMMessage? anchorMessage,
      NIMQueryDirection direction = NIMQueryDirection.desc}) async {
    List<int> typeList = messageType
        .map((e) => V2NIMMessageTypeConverter(messageType: e).toValue())
        .toList();
    ...
    return NimCore.instance.messageService.getMessageListEx(option);
  }

  ///根据关键字搜索消息
  static Future<List<NIMMessage>?> searchMessageByKeyword(String keyWord,
      String sessionId, NIMConversationType conversationType) async {
    NIMMessageSearchParams params = NIMMessageSearchParams(
        keyword: keyWord, sortOrder: NIMSortOrder.sortOrderDesc);
