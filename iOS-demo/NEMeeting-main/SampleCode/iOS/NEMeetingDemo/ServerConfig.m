...
NSString *const kAppKey = @"4727023efa991d31d61b3b32e819bd5b";
...
- (instancetype)init:(NSString *)appkey sdkServerUrl:(NSString *)sdkServerUrl {
  self = [super init];
  if (self) {
    self.appKey = appkey;
...
+ (ServerConfig *)current {
  ...
  if (currentConfig == nil) {
    ...
    NSString *sdkServerUrl = [ServerConfig ifEmpty:[ServerConfig customSDKServerUrl]
                                          fallback:candicate.sdkServerUrl];
...
+ (NSString *)serverType {
  return [ServerConfig ifEmpty:[[NSUserDefaults standardUserDefaults] stringForKey:kServerType]
                      fallback:@"online"];
}
