...
/**
 * @if English
 * Callback for the play streaming duration.
 * @par When to use
 * This callback is triggered when the play streaming duration changes.
 * @param streamId The ID of the stream.
 * @param totalDurationS The total duration of the play streaming process.
 * @param currentPtsS The current PTS of the play streaming process.
 * @since 5.9.15
 * @endif
 * @if Chinese
 * 拉流过程中播放时长改变的回调。
 ...
 * 拉流过程中播放时长改变时会触发该回调。
 * @param streamId 流 ID。
 * @param totalDurationS 拉流过程中播放的总时长。
 * @param currentPtsS 拉流过程中当前的 PTS。
 * @since 5.9.15
 * @endif
 */
- (void)onNERtcEnginePlayStreamingDuration:(NSString*)streamId totalDurationS:(int64_t)total_duration_s currentPtsS:(int64_t)current_pts_s;
