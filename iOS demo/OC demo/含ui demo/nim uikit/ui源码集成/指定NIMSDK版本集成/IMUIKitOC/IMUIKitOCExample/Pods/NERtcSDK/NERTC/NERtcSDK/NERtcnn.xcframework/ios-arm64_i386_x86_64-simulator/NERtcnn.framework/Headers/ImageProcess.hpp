...
public:
    ...
    /**
    * resize 4x upsample, this need scaleW equal to 4, and scaleH between [3.5, 4.5]
    * @param srcData
    * @param srcWidth
    * @param srcHeight
    * @param dstData
    ...
    * @param dstHeight
    */
    int resizeUp4(void* srcData, int srcWidth, int srcHeight, void* dstData, int dstWidth, int dstHeight);

    /**
     * alphaFuse, this now only works for 3 channel Type uint8 data
     * @param srcImg1 front img
     * @param srcImg2 background img
     * @param alpha float alpha img
     * @param dst fuse out image, this support inplace IN the srcImg1 or srcImg2
     * @param width, height, channel  this should be same for all srcImg and alpha img
     */
    int alphaFuse(uint8_t* srcImg1, uint8_t* srcImg2, float* alpha, uint8_t* dstImg, int width, int height, int channel);
