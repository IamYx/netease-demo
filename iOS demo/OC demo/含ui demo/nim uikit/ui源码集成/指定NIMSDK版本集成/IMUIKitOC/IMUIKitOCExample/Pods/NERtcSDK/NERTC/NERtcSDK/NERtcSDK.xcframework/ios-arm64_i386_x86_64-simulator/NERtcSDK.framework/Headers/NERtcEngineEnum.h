...
typedef NS_ENUM(int, NERtcVideoLowlightEnhanceLevel) {
  /**
   * @if Chinese
   * 低光环境增强级别：优先进行画质增强，暗部增强明显
   * @endif
   */
  kNERtcVideoLowlightEnhanceLevelQuality = 1, // quality level
  /**
   * @if Chinese
   * 低光环境增强级别：只增强暗部
   * @endif
   */
  kNERtcVideoLowlightEnhanceLevelBalance = 2, // balance level
  /**
   * @if Chinese
   * 低光环境增强级别：根据画面整体亮度决定是否进行增强
   * @endif
   */
  kNERtcVideoLowlightEnhanceLevelAuto = 3, // auto level
};
