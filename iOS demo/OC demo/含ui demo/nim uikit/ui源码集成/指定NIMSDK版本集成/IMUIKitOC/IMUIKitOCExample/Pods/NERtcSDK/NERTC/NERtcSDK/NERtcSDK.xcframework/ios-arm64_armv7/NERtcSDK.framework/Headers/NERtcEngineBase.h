 ...
 * @endif
 */
NERTC_EXPORT @interface NERtcLLMRequestParams : NSObject

/**
 * @if Chinese
 * 自定义的 LLM 任务 ID。字母、数字、下划线组成的 64 位以内的字符串。请保证此 ID 唯一。
该字段为可选，支持不设置或设置为空。在这种情况下，LLM 任务 ID 由 SDK 生成并管理，并将在用户离开时自动清除。
 * @endif
 */
 ...
 * @endif
 */
@property (nonatomic, copy, nullable) NSString *mediaContent;
...
/**
 * @if Chinese
 * 传入文本信息或外部问题时，处理的优先级, 默认为 1
 * 1:高优先级。传入信息直接打断交互，进行处理。
 * 2:中优先级。等待当前交互结束后，进行处理。
 * 3:低优先级。如当前正在发生交互，直接丢弃。
 * @endif
 */
@property (nonatomic, assign) int interruptMode;
...
/**
 * @if English
 * The normalized rectangle area (0~1) where the mosaic effect is applied.
 * @endif
 * @if Chinese
 * 需要应用马赛克效果的归一化区域（0~1）。
 */
NERTC_EXPORT @interface NERtcFrameNormalizedRect : NSObject
...
/**
 * @if English
 * The configuration for external video blending.
 * @endif
 * @if Chinese
 * 外部视频混合配置。
 * @endif
 */
NERTC_EXPORT @interface NERtcVideoExternalBlendConfig : NSObject

/**
 * @if English
 * The x coordinate of the normalized rectangle area (0~1) where the mosaic effect is applied.
 * @endif
 * @if Chinese
 * 需要应用马赛克效果的归一化区域（0~1）。
 */
...
/**
 * @if English
 * The y coordinate of the normalized rectangle area (0~1) where the mosaic effect is applied.
 * @endif
 * @if Chinese
 * 需要应用马赛克效果的归一化区域（0~1）。
 */
...
/**
 * @if English
 * The width of the normalized rectangle area (0~1) where the mosaic effect is applied.
 * @endif
 * @if Chinese
 * 需要应用马赛克效果的归一化区域（0~1）。
 */
...
/**
 * @if English
 * The height of the normalized rectangle area (0~1) where the mosaic effect is applied.
 * @endif
 * @if Chinese
 * 需要应用马赛克效果的归一化区域（0~1）。
 */
...
/**
 * @if English
 * The scale mode of the video.
 * @endif
 * @if Chinese
 * 视频缩放模式。
 * kNERtcVideoRenderScaleFit: 缩放模式，保持原始比例
 * kNERtcVideoRenderScaleFullFill: 填充模式,宽高比例不一致时，会进行缩放
 * kNERtcVideoRenderScaleCropFill: 裁剪模式,宽高比例不一致时，会进行裁剪
 */
@property (nonatomic, assign) NERtcVideoRenderScaleMode scaleMode;
