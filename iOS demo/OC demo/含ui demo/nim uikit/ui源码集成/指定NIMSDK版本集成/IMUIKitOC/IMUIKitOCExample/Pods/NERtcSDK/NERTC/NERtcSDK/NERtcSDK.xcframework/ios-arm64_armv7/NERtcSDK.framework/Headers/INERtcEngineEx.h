...
/**
 * @if English
 * Seek to a specified position for play streaming.
 * @since V5.9.15
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 ...
 * @par Example
 * @code
 * [[NERtcEngine sharedEngine] seekPlayStreaming:streamId positionMs:positionMs];
 * @endcode
 * @return
 * - 0 (kNERtcNoError): A successful response.
 * - Others: An error response.
 ...
 * @endif
 */
- (int)seekPlayStreaming:(NSString *)streamId positionMs:(int64_t)positionMs;
...
/**
 * @if English
 * Request llm in a room.
 * <br>After the method is successfully called, the members in the room can receive responses from llm.
 * @param dstUid            specifies the uid of AI in the meeting can be set to 0 by default, and the SDK will automatically search for it.
 * @param params            specifies the LLM request parameters, see {@link NERtcLLMRequestParams}.
 * @param compeltion        Operation result callback, indicating the request status, see {@link NERtcLLMRequestCompletion}.
 * @return {@code 0} A value of 0 returned indicates that the method call is successful. Otherwise, the method call fails.
 * @endif
 * @if Chinese
 * 请求llm。
 ...
 *      <td>dstUid</td>
 *      <td>{uint64_t}</td>
 *      <td>AI在会中的uid，默认可以写0，sdk会自动查找。</td>
 *  </tr>
 *  <tr>
 *      <td>params</td>
 *      <td>{@link NERtcLLMRequestParams}</td>
 *      <td>llm请求参数。</td>
 *  </tr>
 *  <tr>
 *      <td>compeltion</td>
 *      <td>{@link NERtcLLMRequestCompletion}</td>
 *      <td>操作结果回调，表示请求状态。</td>
 *  </tr>
 * </table>
 * @par 示例代码
 ...
 * [[NERtcEngine sharedEngine] requestLLM:0 params:params compeltion:^(NSString * _Nonnull taskId, int code, NSString * _Nullable errorMsg) {
 *   NSLog(@"taskId:%@, code:%d, errorMsg:%@", taskId, code, errorMsg);
 * }];
 * @endcode
 * @par 相关回调
 * {@link NERtcLLMRequestCompletion}：请求结果回调。
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 *      - 30003（kNERtcErrInvalidParam）：错误的参数。
 ...
 * @endif
 */
- (int)requestLLM:(uint64_t)dstUid params:(NERtcLLMRequestParams *)params compeltion:(NERtcLLMRequestCompletion)compeltion;

#pragma mark - Video Mosaic

/**
 * @if English
 * Set local video mosaic.
 * <br>This method allows you to enable or disable mosaic (pixelation) effect on the local video stream, and specify the area to apply the effect.
 * @since V5.9.15
 * @par When to use
 * Please call this method after the engine initialization.
 * @note
 * - There is no need to join an RTC room when calling this method.
 * - The mosaic effect is applied to the specified area of the local video stream.
 * @param streamType The video stream channel type. See {@link NERtcStreamChannelType}.
 * @param enabled Whether to enable the mosaic effect. YES to enable, NO to disable.
 * @param rect The normalized rectangle area (0~1) where the mosaic effect is applied. See {@link NERtcFrameNormalizedRect}.
 * @return
 * - 0: Success.
 * - Other: Failure.
 * @endif
 * @if Chinese
 * 设置本地视频马赛克效果。
 * <br>通过此接口可以对本地视频流指定区域开启或关闭马赛克（像素化）效果。
 * @since V5.9.15
 * @par 使用时机
 * 请在引擎初始化后调用本方法。
 * @note
 * - 调用本方法无需加入房间。
 * - 马赛克效果仅作用于本地视频流的指定区域。
 * @param streamType 视频流通道类型。详见 {@link NERtcStreamChannelType}。
 * @param enabled 是否开启马赛克效果。YES 表示开启，NO 表示关闭。
 * @param rect 需要应用马赛克效果的归一化区域（0~1），详见 {@link NERtcFrameNormalizedRect}。
 * @return
 * - 0（kNERtcNoError）：方法调用成功。
 * - 其他：方法调用失败。
 * @endif
 */
- (int)setLocalVideoMosaic:(BOOL)enabled streamType:(NERtcStreamChannelType)streamType rect:(NERtcFrameNormalizedRect *)rect;

/**
 * @if English
 * Set remote video mosaic.
 * <br>This method allows you to enable or disable mosaic (pixelation) effect on a specified remote user's video stream, and specify the area to apply the effect.
 * @since V5.9.15
 * @par When to use
 * Please call this method after the engine initialization and after joining a room.
 * @note
 * - The mosaic effect is applied to the specified area of the remote user's video stream.
 ...
 * @if Chinese
 * 设置远端视频马赛克效果。
 * <br>通过此接口可以对指定远端用户的视频流指定区域开启或关闭马赛克（像素化）效果。
 * @since V5.9.15
 * @par 使用时机
 * 请在引擎初始化并加入房间后调用本方法。
 * @note
 * - 马赛克效果仅作用于指定远端用户视频流的指定区域。
 ...
 * @endif
 */
- (int)setRemoteVideoMosaic:(BOOL)enabled userId:(uint64_t)userId streamType:(NERtcStreamChannelType)streamType rect:(NERtcFrameNormalizedRect *)rect;

#pragma mark - Video Blend

/**
 * @if English
 * Starts the process of external video blending on a specified video stream channel.
 * <br>Call this method to begin mixing external video frames into the specified stream channel.
 * @since V5.9.15
 * @note
 * - Call this method after engine initialization and after starting the relevant video stream.
 * - You must call `stopBlendExternalVideo:` to end the blend session when no longer needed.
 * @param streamType The type of video stream channel to blend to. See {@link NERtcStreamChannelType}.
 * @param blendConfig The configuration for external blending. See {@link NERtcVideoExternalBlendConfig}.
 * @return
 * - 0: Success.
 * - Other: Failure (for example, incorrect parameters or invalid state).
 * @endif
 * @if Chinese
 * 开始外部视频混合处理。
 * <br>通过本方法可在指定视频流通道类型上开启外部视频混合流程，需要传入混合参数配置。
 * @since V5.9.15
 * @note
 * - 请在引擎初始化后并开启对应视频流通道后调用。
 * - 混合处理完成后需调用 `stopBlendExternalVideo:` 停止混合流程。
 * @param streamType 视频流通道类型，详见 {@link NERtcStreamChannelType}。
 * @param blendConfig 外部混合参数配置，详见 {@link NERtcVideoExternalBlendConfig}。
 * @return
 * - 0：方法调用成功。
 * - 其他：方法调用失败（如参数错误或状态非法）。
 * @endif
 */
- (int)startBlendExternalVideo:(NERtcStreamChannelType)streamType
                   blendConfig:(NERtcVideoExternalBlendConfig *)blendConfig;

/**
 * @if English
 * Stops external video blending on the specified video stream channel.
 * <br>Call this method to end the external video blending process for the specified stream channel.
 * @since V5.9.15
 * @note
 * - Call this method after starting external video blending, or when the blend session should end.
 * @param streamType The type of video stream channel. See {@link NERtcStreamChannelType}.
 * @return
 * - 0: Success.
 * - Other: Failure (for example, mixing was not started or invalid parameters).
 * @endif
 * @if Chinese
 * 停止外部视频混合处理。
 * <br>调用此方法结束指定视频流通道类型的外部视频混合流程。
 * @since V5.9.15
 * @note
 * - 请在已开启外部视频混合后，或需要结束混合流程时调用。
 * @param streamType 视频流通道类型，详见 {@link NERtcStreamChannelType}。
 * @return
 * - 0：方法调用成功。
 * - 其他：方法调用失败（如未开始混合或参数错误）。
 * @endif
 */
- (int)stopBlendExternalVideo:(NERtcStreamChannelType)streamType;

/**
 * @if English
 * Pushes an external video frame to the blending process for the specified stream channel.
 * <br>Use this method to provide a video frame to be mixed into the target channel started with external blending.
 * @since V5.9.15
 * @note
 * - Call this method only after successfully starting external video blending by `startBlendExternalVideo:blendConfig:`.
 * - You should push frames continuously as required by your use case.
 * @param streamType The type of video stream channel to blend into. See {@link NERtcStreamChannelType}.
 * @param frame The video frame to be blended. See {@link NERtcVideoFrame}.
 * @return
 * - 0: Success.
 * - Other: Failure (for example, blending not started or invalid frame data).
 * @endif
 * @if Chinese
 * 推送外部要混合的视频帧到混合流程中。
 * <br>用于在已开启外部视频混合处理的流程中持续推送待混合的视频帧。
 * @since V5.9.15
 * @note
 * - 仅在已通过 `startBlendExternalVideo:blendConfig:` 成功开启混合流程后调用本方法。
 * - 混合帧需根据实际需求持续推送。
 * @param streamType 视频流通道类型，详见 {@link NERtcStreamChannelType}。
 * @param frame 待混合的视频帧数据，详见 {@link NERtcVideoFrame}。
 * @return
 * - 0：方法调用成功。
 * - 其他：方法调用失败（如未开启混合流程或帧数据不合法）。
 * @endif
 */
- (int)pushBlendExternalVideoFrame:(NERtcStreamChannelType)streamType
                             frame:(NERtcVideoFrame *)frame;
 ...
 * @endif
 */
- (int)enableVideoDenoise:(BOOL)enabled streamType:(NERtcStreamChannelType)streamType;
 ...
 * @param streamType 流类型
 * @param enabled 是否开启
 * @param level 低光环境增强级别
 * @return
 * - 0: 方法调用成功
 * - 其他: 调用失败
 * @endif
 */
- (int)enableVideoLowlightEnhance:(BOOL)enabled level:(NERtcVideoLowlightEnhanceLevel)level streamType:(NERtcStreamChannelType)streamType;
